...
TopologyGraphLayout.prototype = {
  ...
  tick: function() {
    ...
    this.link.attrs(function(d) { return { x1: d.source.x, y1: d.source.y, x2: d.target.x, y2: d.target.y }; });
    this.node.attr("transform", function(d) { return "translate(" + d.x + "," + d.y + ")"; });

    this.linkLabel.attrs(function(d) {
      return  {x: (d.link.source.x + d.link.target.x) / 2, y: (d.link.source.y + d.link.target.y) / 2 };
    });
  ...
  collapseNode: function(n, group) {
    ...
    for (i in edges) {
      ...
      if (members.indexOf(e.source) < 0 || members.indexOf(e.target) < 0) {
        ...
        if (source.group.owner.visible && target.group.owner.visible) {
